...
function findAlerts(curSuite, prevSuite, threshold, checkHostName, withRepetitions) {
    ...
    if (checkHostName && curSuite.host_name !== prevSuite.host_name) {
        throw new Error(`The current and previous benchmarks were not run on the same machine: ${curSuite.host_name} != ${prevSuite.host_name}`);
    }
    const names = [];
    for (const current of curSuite.benches) {
        const prev = prevSuite.benches.find((b) => {
            if (withRepetitions) {
                if (current.name.endsWith("_mean") || current.name.endsWith("_median") || current.name.endsWith("_stddev")) {
                    return false;
                }
            }
        ...
        if (withRepetitions) {
            ...
            if (ratio > threshold) {
                core.warning(`Performance alert! Previous minimum value was ${prevMin} and current minimum value is ${curMin}.` +
...
async function handleAlert(benchName, curSuite, prevSuite, config) {
    const { withRepetitions, alertThreshold, githubToken, commentOnAlert, failOnAlert, alertCommentCcUsers, failThreshold, checkHostName, } = config;
