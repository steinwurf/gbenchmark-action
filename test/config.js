...
describe('configFromJobInput()', function () {
    ...
    const validation_tests = [
        ...
        {
            what: 'wrong GitHub branch name',
            inputs: { ...defaultInputs, 'gh-branch': '' },
    ...
    for (const test of validation_tests) {
        it('validates ' + test.what, async function () {
            ...
            await assert_1.strict.rejects(configFromJobInput, test.expected);
        });
    }
    ...
    const returnedConfigTests = [
        ...
        {
            what: 'with specified GitHub branch',
            inputs: { ...defaultInputs, 'gh-branch': 'master' },
            expected: { ...defaultExpected, ghBranch: 'master' },
        },
        ...[
            ['150%', 1.5],
            ['0%', 0],
    ...
    for (const test of returnedConfigTests) {
        it('returns validated config with ' + test.what, async function () {
            ...
            assert_1.strict.equal(actual.name, test.expected.name);
            assert_1.strict.equal(actual.ghBranch, test.expected.ghBranch);
            assert_1.strict.equal(actual.githubToken, test.expected.githubToken);
            assert_1.strict.equal(actual.commentOnAlert, test.expected.commentOnAlert);
            assert_1.strict.equal(actual.failOnAlert, test.expected.failOnAlert);
            assert_1.strict.equal(actual.alertThreshold, test.expected.alertThreshold);
            assert_1.strict.deepEqual(actual.alertCommentCcUsers, test.expected.alertCommentCcUsers);
            assert_1.strict.ok(path.isAbsolute(actual.outputFilePath), actual.outputFilePath);
            assert_1.strict.ok(path.isAbsolute(actual.benchmarkDataDirPath), actual.benchmarkDataDirPath);
            assert_1.strict.equal(actual.maxItemsInChart, test.expected.maxItemsInChart);
            if (test.expected.failThreshold === null) {
                assert_1.strict.equal(actual.failThreshold, test.expected.alertThreshold);
            }
            else {
                assert_1.strict.equal(actual.failThreshold, test.expected.failThreshold);
            }
            if (test.expected.hasExternalDataJsonPath) {
                assert_1.strict.equal(typeof actual.externalDataJsonPath, 'string');
                assert_1.strict.ok(path.isAbsolute(actual.externalDataJsonPath), actual.externalDataJsonPath);
            }
            else {
                assert_1.strict.equal(actual.externalDataJsonPath, undefined);
            }
        });
    }
    it('resolves relative paths in config', async function () {
        ...
        assert_1.strict.equal(config.name, 'Benchmark');
        assert_1.strict.ok(path.isAbsolute(config.outputFilePath), config.outputFilePath);
        assert_1.strict.ok(config.outputFilePath.endsWith('out.txt'), config.outputFilePath);
        assert_1.strict.ok(path.isAbsolute(config.benchmarkDataDirPath), config.benchmarkDataDirPath);
        assert_1.strict.ok(config.benchmarkDataDirPath.endsWith('output'), config.benchmarkDataDirPath);
    });
    it('does not change abusolute paths in config', async function () {
        ...
        const config = await configFromJobInput();
        assert_1.strict.equal(config.outputFilePath, outFile);
        assert_1.strict.equal(config.benchmarkDataDirPath, dataDir);
    });
    it('resolves home directory in output directory path', async function () {
        ...
        const config = await configFromJobInput();
        assert_1.strict.ok(path.isAbsolute(config.outputFilePath), config.outputFilePath);
        assert_1.strict.equal(config.outputFilePath, path.join(absCwd, 'out.txt'));
        assert_1.strict.ok(path.isAbsolute(config.benchmarkDataDirPath), config.benchmarkDataDirPath);
        assert_1.strict.equal(config.benchmarkDataDirPath, path.join(absCwd, 'outdir'));
    });
});
